...
?>		
			...
			$('.add_custom_field-button').click(function(){
				...
				else
				{
					...
					if($confirm)
					{
						...
						//ajax delete data
						$.post("<?php echo get_template_directory_uri();?>/functions/delete_custom_data_col.php",{'column': $sp},function(data){
							if(data == 'success')
							{
	...
	<form method="post" action="options.php" enctype="multipart/form-data">
		...
		<div id="le_floatnav">
			<ul>
				...
				<?php 
					foreach ($optionspanel_array as $key => $value) {
						echo '<li><a href="#' . str_replace(' ', '', $key) . '">' . $key . '</a></li>';
					}
				?>
	...
	<?php 
	if(isset($can_chimp_sync)): 
	if($can_chimp_sync && get_option('lefx_mclistid') != '' && get_option('lefx_mcapikey') != ''): ?>
	<div id="chimpSync">
		<?php 
			$chimplists = $api->lists(array(),0,100);
			$chimplists = $chimplists['data'];
	...
	?>
	<?php 
	if(isset($can_aweber_sync)):
	if($can_aweber_sync && get_option('lefx_aweberlistid') != '' && get_option('lefx_aweberconsumerkey') != '' && get_option('lefx_aweberconsumersecret') != ''):?>
	<?php
